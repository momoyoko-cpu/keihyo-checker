...
export async function buildReport(results, meta) {
  ...
  // ===== 表紙・サマリー =====
  doc.addPage();
  doc.fontSize(20).fillColor("#202124").text("景品表示法 チェックレポート", { align: "left" });
  doc.moveDown(0.5);
  ...
  doc.moveDown(0.3);
  doc.fontSize(11);
  doc.fillColor(SEVERITY_COLOR["高"]).text(`重要度 高: ${counts["高"] || 0} 件`);
  doc.fillColor(SEVERITY_COLOR["中"]).text(`重要度 中: ${counts["中"] || 0} 件`);
  doc.fillColor(SEVERITY_COLOR["低"]).text(`重要度 低: ${counts["低"] || 0} 件`);
  // 法令別の内訳
  const byLaw = {};
  ...
  for (const r of results) {
    ...
    for (const f of r.findings) {
      ...
      doc.fontSize(11).fillColor(color).text(`【${f.severity}】${lawTag}${f.category}  #${n}`);
